...
describe('progressionTools', () => {
  ...
  describe('award_experience', () => {
    it('single target: award 300 XP to one character - XP increases', () => {
      const char = makeCharacter({ experienceToNextLevel: 5000 });
      server.joinParty(char);
      server.awardExperience(300, 'hero-1');
      const updated = server.getTarget('hero-1');
      expect(updated).toBeDefined();
      expect(updated.experience).toBeGreaterThan(0);
    });

    it('party-wide split: award 300 XP to all - each gets their share', () => {
      const char1 = makeCharacter({ id: 'hero-1', name: 'Valerius', experienceToNextLevel: 5000 });
      const char2 = makeCharacter({ id: 'hero-2', name: 'Buddy', experienceToNextLevel: 5000 });
      server.joinParty(char1);
      server.joinParty(char2);
      server.awardExperience(300, 'party');
      const state = server.getFullState();
      const p1 = state.party.find(c => c.id === 'hero-1');
      expect(p1).toBeDefined();
      const p2 = state.party.find(c => c.id === 'hero-2');
      expect(p2).toBeDefined();
      expect(p1.experience).toBeGreaterThan(0);
      expect(p2.experience).toBeGreaterThan(0);
      expect(p1.experience).toBe(p2.experience);
    });

    it('level-up trigger: XP crosses threshold - leveledUp flag', () => {
      const char = makeCharacter({ experience: 299 });
      server.joinParty(char);
    ...
    it('cap at 20: award XP to level 20 character - does not exceed cap', () => {
      const char = makeCharacter({ level: 20, experience: 378000, experienceToNextLevel: 0 });
      server.joinParty(char);
      server.awardExperience(50000, 'hero-1');
      const updated = server.getTarget('hero-1');
      expect(updated).toBeDefined();
      expect(updated.level).toBe(20);
    });

    it('during combat: award while combat active - works', () => {
      const char = makeCharacter({ experienceToNextLevel: 5000 });
      server.joinParty(char);
      server.add_enemy('Goblin');
      server.start_combat();
      server.awardExperience(100, 'hero-1');
      const updated = server.getTarget('hero-1');
      expect(updated).toBeDefined();
      expect(updated.experience).toBeGreaterThan(0);
    });

    it('0 XP: award 0 - no level-up', () => {
      const char = makeCharacter();
      server.joinParty(char);
      const result = server.awardExperience(0, 'hero-1');
