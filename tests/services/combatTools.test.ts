...
describe('combatTools', () => {
  ...
  describe('player_attack', () => {
    ...
    it('extractRollData produces well-formed RollData (no NaN) (regression)', async () => {
      ...
      expect(Number.isNaN(attackCard?.dieRoll ?? NaN)).toBe(false);
      expect(Number.isNaN(attackCard?.total ?? NaN)).toBe(false);
      expect(Number.isNaN(attackCard?.modifier ?? NaN)).toBe(false);
      expect(Number.isNaN(attackCard?.dc ?? NaN)).toBe(false);
