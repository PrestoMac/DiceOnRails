...
- **`hooks/usePresence.ts`**: subscribes to `presence-${campaignId}-typing`. Each client tracks `{ userId, characterId, name, portraitUrl, isTyping, lastActive }`. Typing is announced on the **first keystroke** and stays active for **10s after the last keystroke** (no flickering — subsequent strokes within the window just reset the idle timer). Stale entries pruned after 30s. Excludes the local user; dedups by `userId`. **Stable channel lifetime**: the channel effect depends on the primitive `myCharacter?.id` (NOT the deep-cloned `Character` object, whose identity flips every `gameState` sync); fresh character data is read inside `buildProfile` via a `myCharacterRef` updated in a separate small effect. **Hardened against WebSocket drops**: auto-reconnect with exponential backoff (1s→30s cap) on `CHANNEL_ERROR`/`TIMED_OUT`/`CLOSED`; re-subscribe on `navigator.online`; re-broadcast on tab visibility change; periodic heartbeat every 10s (only when typing); `track()` awaited with one retry; debug logging gated by `isDebugMode`. Listens to `sync`, `join`, and `leave` events for immediate updates.
...
- `components/BattleMapPanel.tsx` renders the canvas with **coordinate labels** (column/row numbers), **melee-range hover ring** (dashed green at 5 ft), current-turn pulse, dead-token greying, and drag-and-drop with **speed validation**, **token ownership**, and **turn-based restriction**: players can only drag their own character token, and only during their own turn in combat. Invalid drops (exceeding speed) are blocked — the token snaps back and the cell renders with a red fill + border. The canvas cursor changes to `default` when it's not the player's turn.
