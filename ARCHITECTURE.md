...
- **`computeXp(trigger, ctx)`** — pure: returns the XP amount for a given trigger + context bundle. 6 triggers: `combat` (CR-based), `skill` (DC-bracketed, nat 20 = ×2), `explore` (first-visit, `significance`-tiered), `quest` (`difficulty`-tiered), `lore` (flat 10), `roleplay` (LLM-tagged via `roleplay: 'dialogue'\|'creative'` or `xp` param, clamped 1-10).
