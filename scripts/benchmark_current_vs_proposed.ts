...
function currentExecTool(s: MockMCPServer, name: string, args: Record<string, unknown>): Promise<MCPResponse> {
  switch (name) {
    ...
    case 'move_to': return s.move_to(String(args.location_name || 'Unknown'), String(args.description || ''), args.targetId as string, args.skillCheck as { skill_name?: string; difficulty?: number; onSuccess?: unknown } | undefined);
...
function proposedExecTool(s: MockMCPServer, name: string, args: Record<string, unknown>): Promise<MCPResponse> {
  switch (name) {
    ...
    case 'move_to': {
      const r = s.move_to(String(args.location_name || 'Unknown'), String(args.description || ''), args.targetId as string, args.skillCheck as { skill_name?: string; difficulty?: number; onSuccess?: unknown } | undefined);
...
async function runHarness(
  ...
  for (iters = 0; iters < maxIters; iters++) {
    ...
    const hasInlineNarration = resp.toolCalls.some((tc) =>
      tc.name !== 'narrate_turn' && tc.args?.narration
    );
