...
| `move_to` | travel | Simple location move + optional `skillCheck`. Supports `END_OF_TURN_PROPERTIES`. Travel legs are capped at `MAX_MOVE_TO_LEG_MINUTES` (240m); the engine rejects longer legs to force chunked journeys with `long_rest` stops. |
