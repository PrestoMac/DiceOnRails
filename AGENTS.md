...
- **Narration fallback chain** (2 tiers, in `useGameActions.ts`): `inlineNarration` (from agent loop) → `generateNarration` retry (must be ≥25 chars post-sanitize; `useGameActions.ts:229`) → literal `"The adventure continues..."` (`useGameActions.ts:245`). `resolveNarration` (`useGameActions.ts:157-162`) is a degenerate stub that does NO LLM work — it just returns `inlineNarration ?? 'The adventure continues...'`. The 4-tier chain with `generateTightNarration`/`buildDeterministicNarration` previously documented here does NOT exist in code.
...
- **`extractRollData`** (`narration.ts:20`): extracts structured `RollData` from tool results for UI display. Covers `roll_dice`, `check_skill`, `player_attack`, `cast_spell`, `make_save`, `roll_death_save`, `inflict_damage`, `use_resource`, and `next_turn` (returns a `RollData[]` — one attack card + one damage card per resolved enemy attack). Return type is `RollData | RollData[] | undefined`.
...
- **`dispatchToolRolls`** (`gameActionHelpers.ts:83`): fires the `<DiceRollModal>` popup per tool. Handles `roll_dice`, `player_attack`, `check_skill`, `make_save`, `roll_death_save`, `cast_spell`, `use_resource`, `inflict_damage`, `start_combat`, and `next_turn` (loops `attackResults[]`, staggered `setTimeout` fire-and-forget — does not block the agent loop).
