...
- **`ctxRef` not re-hydrated between turns**: when another player processes a turn, the local `ctxRef` (frozen history, checkpoints) is not updated until the local player processes their own turn. Minor context duplication possible (messages already frozen by the other player get re-frozen once). Self-corrects after one local processing cycle. No correctness impact.
