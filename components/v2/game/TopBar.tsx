      ...
      <div className="flex items-center gap-1.5">
        {recentActivity.length > 0 && (
          ...
          >
            <span className="relative inline-flex">
              <IconButton icon="fa-bell" tip="Recent party activity" />
        ...
        {shareId && (
          <span className="hidden sm:inline-flex items-center gap-1 pl-2.5 pr-1 py-1 rounded-full bg-obsidian-800/80 border border-white/[0.08]">
            <i className="fas fa-share-nodes text-[13px] text-arcane-400" aria-hidden="true" />
            <span className="text-[10px] font-mono text-parchment-dim max-w-[72px] truncate">{shareId}</span>
            <IconButton icon="fa-copy" tip="Copy share code" onClick={copyShareId} size="sm" />
          </span>
        )}
        ...
        {showLogout && <IconButton icon="fa-arrow-right-from-bracket" tip="Log out" variant="danger" onClick={onLogout} />}
