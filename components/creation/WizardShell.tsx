...
const WizardShell: React.FC<WizardShellProps> = ({
  ...
  const handleFinalize = () => {
    ...
    const { name, selectedRace, selectedClass, stats, inventory, goldPool, level, allocatedSkills, asiFeatSlots, selectedSubclassId, selectedCantrips, selectedSpells, draconicAncestry, halfElfChoice1, halfElfChoice2, backstory, selectedLocation } = wizardState;
    if (!name.trim()) { setFinalizeError("Your character must have a name before beginning their chronicle."); return; }
    if (remainingSkillPoints > 0) { setFinalizeError(`You still have ${remainingSkillPoints} unspent skill points.`); return; }
    for (let i = 0; i < asiFeatSlots.length; i++) {
      if (asiFeatSlots[i].type === null) { setFinalizeError(`Please complete Feats & Ability Improvements slot ${i + 1}.`); return; }
    }
    const loc = isNewCampaign ? selectedLocation : (campaignStartingLocation || FALLBACK_STARTING_LOCATION);
    ...
    onComplete({
      ...
      hp: { current: selectedClass.hpBase + conMod + (selectedClass.hpPerLevel + conMod) * (level - 1), max: selectedClass.hpBase + conMod + (selectedClass.hpPerLevel + conMod) * (level - 1) },
      stats: fs, inventory, currency: { gp: Math.floor(goldPool), sp: Math.round((goldPool % 1) * 10), cp: 0 },
      location: loc?.name || '', experience: 0, experienceToNextLevel: 0,
      unusedStatPoints: (level - 1) * 2, maxHpBonus: 0, hitDice: { current: level, max: level },
      skills: updatedSkills, unusedSkillPoints: remainingSkillPoints,
