...
class ErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, info: React.ErrorInfo) {
    console.error('[ErrorBoundary] Caught:', error, info, 'Build timestamp:', import.meta.env.VITE_BUILD_TIME || 'dev');
  }
      ...
      return this.props.fallback || (
        <div className="flex h-screen w-full items-center justify-center bg-stone-950 p-8">
          <div className="max-w-md text-center">
            ...
            <p className="text-stone-400 mb-2 font-mono text-sm">
              {this.state.error?.message || 'An unexpected error occurred.'}
            </p>
            <p className="text-stone-600 mb-6 font-mono text-xs">
