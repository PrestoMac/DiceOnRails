import { Character, StartingLocation, FeatSelection } from '../types';
...
import { getMod, getClassDef, getRaceDef, recalculateResourcePools } from './classEngine';
...
export function buildCharacterFromWizard(
  ...
  if (!name.trim()) { errors.push("Your character must have a name before beginning their chronicle."); return { character: null as any, errors }; }

  const loc = isNewCampaign ? wizard.selectedLocation : (campaignStartingLocation || FALLBACK_STARTING_LOCATION);
  if (isNewCampaign && !loc) { errors.push("Please select a starting location."); return { character: null as any, errors }; }
